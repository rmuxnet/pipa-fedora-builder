...
image_mnt='mnt_image'
date=$(date +%Y%m%d)
image_name=alioth-fedora-${date}-1
...
mkosi_create_rootfs() {
    ...
    rm -rf $mkosi_rootfs/root/alioth-fedora-builder
}

mount_image() {
    # get last modified image
    image_path=$(find $image_dir -maxdepth 1 -type d | grep -E "/alioth-fedora-[0-9]{8}-[0-9]" | sort | tail -1)
...
make_image() {
    ...
    MKE2FS_DEVICE_PHYS_SECTSIZE=4096 MKE2FS_DEVICE_SECTSIZE=4096 mkfs.ext4 -U "$ROOTFS_UUID" -L 'fedora_alioth' "$image_dir/$image_name/root.img"
